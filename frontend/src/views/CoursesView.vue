      ...
      <main class="flex-1 p-8 animate-fade-in">
        <div class="page-header flex items-start justify-between">
          <div>
            ...
            <p class="page-subtitle">{{ authStore.isDosen ? 'Kelola kelas dan pantau presentasi mahasiswa' : 'Daftar kelas yang tersedia' }}</p>
          </div>
          <button v-if="authStore.isDosen" id="add-course-btn" class="btn-primary" @click="showAddModal = true">
        ...
        <!-- Loading -->
        <div v-if="isLoading" class="flex justify-center py-16"><div class="spinner w-8 h-8"></div></div>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          <div
            ...
            :key="course.id"
            class="card-shadow hover:border-vox-black transition-all"
          >
            <div class="flex items-start justify-between mb-3">
              ...
              <button v-if="authStore.isDosen" @click.stop="deleteCourse(course.id)"
                class="text-vox-midgray hover:text-red-500 transition-colors p-1" title="Hapus kelas">
            ...
            <div class="mt-4 pt-3 border-t border-vox-lightgray flex justify-between items-center">
              ...
              <button v-if="authStore.isDosen"
                @click="selectCourse(course)"
                class="text-xs text-vox-black font-medium hover:underline">Lihat Detail →</button>
            </div>
          </div>
        </div>
        ...
        <div v-if="selectedCourse" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div class="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col m-4">
              ...
              <table v-else class="data-table w-full">
                ...
                <tbody>
                  <tr v-for="p in presentations" :key="p.id">
                    <td>
                      <div class="font-medium text-vox-black text-sm">{{ p.student.name }}</div>
                      <div class="text-vox-midgray text-xs">{{ p.student.nimNip ?? '—' }}</div>
                    </td>
                    <td class="text-vox-black text-sm max-w-[180px]">
                      <div class="truncate">{{ p.title }}</div>
                    </td>
                    <td>
                      <span v-if="p.evaluation" class="font-bold">{{ p.evaluation.overallScore.toFixed(1) }}</span>
          ...
          <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 m-4">
            ...
            <form @submit.prevent="handleAddCourse" class="space-y-4">
              <div>
                ...
                <input id="course-name-input" v-model="newCourseName" type="text" class="input-field"
...
async function deleteCourse(id: string) {
  ...
  try {
    ...
    await loadCourses()
  } catch (e: any) { alert(e.response?.data?.message || 'Gagal menghapus kelas') }
}
...
async function selectCourse(course: any) {
  ...
  } catch { presentations.value = [] }
  finally { loadingPresentations.value = false }
}
